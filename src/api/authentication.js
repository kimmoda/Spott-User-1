...
export async function login (baseUrl, { email, password }) {
  try {
    ...
    return await request.post(null, `${baseUrl}/rest/v003/security/login`, { userName: email, password });
  } catch (error) {
...
export async function loginFacebook (baseUrl, { facebookAccessToken }) {
  try {
    return await request.post(null, `${baseUrl}/rest/v003/security/login`, { facebookAccessToken });
  } catch (error) {
    if (error.message === 'verkeerde gebruikersnaam/password combinatie') {
...
export function register (baseUrl, { email, firstname, lastname, password }) {
  return request.post(null, `${baseUrl}/rest/v003/user/users/register/username`, { email, firstName: firstname, lastName: lastname, password });
}
